...
public:
    ...
    //generators
    // This just creates a new node of type = node_type and return the treenode. That's it. Nothing more!
    static TreeNode* tree_node_factory(NodeType node_type);
    
    // So given a tree pointer, it returns the treenode which corresponds to this poniter (string).
    static TreeNode* tree_node_factory(TreePtr tree_ptr);
    
    // Given, two tree pointers ptr1 and ptr2, this creates an internal node and put these two treenodes as children and returns the newly
    // created treenode (Internal node here). This is used to create root node.
