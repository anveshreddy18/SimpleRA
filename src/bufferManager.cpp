...
{
    BLOCK_ACCESSES++;
    logger.log("BufferManager::writePage::Table");
    Page page(tableName, pageIndex, rows, rowCount);
    page.writePage("Table");
}
 ...
 * @param blockFill
 */
void BufferManager::writePage(string tableName, int pageIndex, vector<int>&blockFill)
{
    BLOCK_ACCESSES++;
    logger.log("BufferManager::writePage::Matrix");
    vector<vector<int>>rows;
    rows.push_back(blockFill);
    int rowCount =1;
    Page page(tableName, pageIndex, rows, rowCount);
