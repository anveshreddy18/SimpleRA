...
Page::Page(string tableName, int pageIndex)
{
    ...
    if(parsedQuery.printType == "Matrix")
    {
        logger.log("Printing matrix");
        Matrix matrix = *matrixCatalogue.getMatrix(tableName);
        int n = matrix.columnCount;
        if(pageIndex == matrix.blockCount-1)
        {
            this->columnCount =  1 + ((long long int)n*(long long int)n - 1) % (long long int)matrix.maxElemsPerBlock;
        }
        else this->columnCount = matrix.maxElemsPerBlock;
 ...
 * @param type
 */
void Page::writePage(string type)
{
    logger.log("Page::writePage::"+type);
...
void Page::editPage(int ind, int val)
{
    this->rows[0][ind] = val;
    this->writePage("Matrix");
}
